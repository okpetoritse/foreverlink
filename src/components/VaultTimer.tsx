...
export default function VaultTimer({ targetDate }: { targetDate: string }) {
  ...
  if (timeLeft.isUnlocked) {
    return (
      <div className="text-[#64FFDA] font-bold animate-pulse tracking-widest text-center">
  ...
  const TimeUnit = ({ label, value }: { label: string, value: number }) => (
    <div className="flex flex-col items-center">
      ...
      <span className="text-[10px] uppercase text-[#D4AF37] tracking-[0.2em] mt-1">{label}</span>
    </div>
  );

  return (
    <div className="bg-[#0A192F] border border-[#D4AF37]/20 rounded-2xl p-8 shadow-[0_0_50px_rgba(212,175,55,0.05)]">
